/**
 * gis_auth.js handles authentication negotiation with ArcGIS servers.
 */
const fs = require("fs");
...
const configuration = require("../config/server_configuration.json");
...
function cacheResponse(arcgisServerResponse) {
  if (!isArcGISError(arcgisServerResponse)) {
    ...
    try {
      const cache = fs.createWriteStream(configuration.cacheFile, {
        flags: "w",
      });
      if (cache) {
    ...
    try {
      ...
      cache.on("end", function (chunk) {
        try {
          ...
          reject(
            new Error(
              "Could not parse response as JSON: " + response.toString()
            )
          );
        }
      });
...
function isArcGISError(arcgisServerResponse) {
  return (
    arcgisServerResponse == null ||
    typeof arcgisServerResponse.error !== "undefined"
  );
}
    ...
    if (!forceRefresh) {
      ...
      getCachedToken()
        .then(
          ...
          function () {
            // cache is not good, get a new token
